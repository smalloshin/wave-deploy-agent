...
| 2026-04-30 | [r44h-strictness-flip-guard](./2026-04-30-r44h-strictness-flip-guard.md) | Active | R44h：兩道防線 — (1) `isStrictnessFlip` 在 pipeline Step 5 擋住 LLM auto-fix 把 `ignoreBuildErrors`/`ignoreDuringBuilds` 從 `true→false` 翻轉（vibe-coded 專案的旗標是故意開的，翻了 build 直接死）；(2) `detectNextMajorVersion` + `stripEslintFromNextConfig` 在 Step 2 偵測 Next ≥ 16 自動砍 `next.config.ts` 的 deprecated `eslint:{}` 欄位（Next 16 移除了該 key，type-check 會炸）；63 個 zero-dep 測試 |
