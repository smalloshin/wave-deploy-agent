...
export interface BuildFailureAnalysis {
  ...
  /** 誰要負責修這個錯 —— user（使用者 code）/ platform（部署平台本身）/ environment（外部：GCP quota、網路）/ unknown */
  ownership: 'user' | 'platform' | 'environment' | 'unknown';
  /** 一句話摘要（繁體中文） */
  summary: string;
  /** 使用者面向訊息：直接告訴使用者「你的 code 哪裡錯，怎麼改」或「這不是你的錯，平台在處理中」 */
  userFacingMessage: string;
  /** 管理員面向訊息：技術細節、infra 問題、deploy-agent 需要補什麼權限／改什麼 code */
  adminFacingMessage: string;
  /** 使用者能自己修嗎？ */
  userActionable: boolean;
  /** 平台需要補 IAM / 改 code / 補資源嗎？ */
  platformActionable: boolean;
  /** 錯誤的根本原因（技術細節，管理員看） */
  rootCause: string;
  /** 具體的修復建議 —— deprecated，新 UI 改讀 userFacingMessage + adminFacingMessage */
  suggestedFix: string;
...
function fallbackAnalysis(
  ...
  return {
    ...
    userFacingMessage: '部署失敗但自動分析功能暫時不可用。請聯繫管理員，或稍後再試。',
    adminFacingMessage: note,
    userActionable: false,
    platformActionable: true,
...
export async function analyzeDeployFailure(
  ...
  if (!hasLog && !hasSource) {
    return {
      category: 'unknown',
      ownership: 'platform',
      summary: '部署失敗，但平台無法取得 build log 做自動分析',
      userFacingMessage: '你的專案部署失敗了，但我們的系統這次沒辦法抓到完整 build log 自動分析原因。\n\n👉 **這不一定是你的 code 問題** —— 平台端已經在處理讀取權限。\n\n**建議動作**：請按右上角「重試流程」重新部署一次。如果再次失敗，新的 log 會寫到可讀的位置，到時點「🤖 用 AI 重新分析」就能看到精確的錯誤原因和修法。',
      adminFacingMessage: 'Build log fetch failed (likely HTTP 403 from Google-managed cloudbuild-logs bucket). 舊 build 的 log 寫到 `gs://{PROJECT_NUMBER}.cloudbuild-logs.googleusercontent.com`（Google 內部管的 bucket，無法授權給 deploy-agent SA）。修法：`deploy-engine.ts` 已加 `options.logsBucket` 指向自有 bucket，新建 build 就能讀。對現存失敗的 transition，必須重新 deploy 一次才能拿到 log。',
      userActionable: true,
      platformActionable: false,
      rootCause: 'Build log 不在可讀 bucket 內（Google managed cloudbuild-logs，無法授權）。',
      suggestedFix: '重新觸發部署，新的失敗會寫到可讀 log bucket。',
      errorLocation: null,
      errorSnippet: null,
      extraObservations: null,
      step,
  ...
  const system = `你是一位資深 DevOps 工程師，專門協助排查雲端部署 pipeline 的失敗。
你的任務是把錯誤拆成「使用者要做的事」和「管理員（平台維運）要做的事」兩個面向。
這很重要：使用者不懂 GCP / IAM / Cloud Build 細節，只想知道「是不是我的錯、我該改什麼」；
管理員才需要看 infra 技術細節。
...
**ownership 責任歸屬**（最重要的判斷）：
- user：錯在使用者 code 或 config（TypeScript error、missing dep、Dockerfile 寫錯、lock file 沒提交、runtime env var 沒帶）→ 使用者要自己修
- platform：錯在部署平台本身（deploy-agent 沒權限、GCS bucket 讀不到、reanalyze 流程 bug、IAM 設定漏）→ 管理員要修
- environment：外部因素（GCP quota 爆、網路中斷、base image 下架、GitHub rate limit）→ 通常等一下重試即可
- unknown：真的判斷不出來

**userFacingMessage**：寫給**使用者**看的訊息。
- 語氣：像在跟朋友說話，清楚直白
- **不要**出現「SA」「IAM」「bucket」「HMAC」「VPC」這類 infra 術語
- 如果是 user 的錯：直接告訴他「你的 X 檔案第 Y 行有問題，改成 Z」，要 actionable
- 如果是 platform/environment 的錯：告訴他「這不是你的錯，平台正在處理 / 請稍後重試」
- 長度：2~5 句話，可以用 markdown（粗體、條列），不用太長

**adminFacingMessage**：寫給**管理員（boss）**看的訊息。
- 語氣：技術、直接、像同事之間對話
- 可以用 infra 術語
- 講清楚：問題在哪層、平台要補什麼（IAM role / code 改哪個檔 / 建什麼資源），或「確認無平台責任」
- 如果是 user 的錯：一句話「user code 問題，無需 platform 動作」就好

**userActionable**：使用者自己能修嗎？user code/config 問題通常是 true；platform/environment 通常是 false
**platformActionable**：平台需要動作嗎？platform 問題 true；純 user 錯 false

**errorLocation**：如果 log 裡有 "檔案:行號"（e.g. "src/app/foo.ts:47" 或 "route.ts:47:67"）請填在這裡，否則 null
**errorSnippet**：從 log 擷取最能說明問題的 3-8 行原文（含檔名/行號/錯誤訊息/周邊程式碼），保留原始格式。沒有就 null
**extraObservations**：log 裡看到但不是主因的警告或副作用（Next.js deprecation、config 過時等），沒有就 null
**suggestedFix**：保留舊欄位做向下相容，內容類似 userFacingMessage 但可以更長，具體到行號

全部用繁體中文，不要寒暄。

回應格式（僅 JSON，不要 markdown fence）：
{"category":"...","ownership":"user|platform|environment|unknown","summary":"一句話摘要","userFacingMessage":"...","adminFacingMessage":"...","userActionable":true|false,"platformActionable":true|false,"rootCause":"根本原因技術說明","suggestedFix":"具體修復步驟","errorLocation":"檔案:行號 或 null","errorSnippet":"log 原文 或 null","extraObservations":"附加觀察 或 null"}`;
...
${hasSource ? `\n使用者專案 context（package.json/tsconfig/錯誤檔案片段等 —— 請優先用這些判斷 root cause、指出 user 具體要改哪一行）：\n\`\`\`\n${sourceBlock}\n\`\`\`` : ''}`;
  ...
  try {
    ...
    const validOwnerships = ['user', 'platform', 'environment', 'unknown'];
    const category = validCategories.includes(parsed.category as string) ? (parsed.category as BuildFailureAnalysis['category']) : 'unknown';
    // ownership 沒給的話靠 category 猜：user_code/dependency/config/runtime → user；network/infra → environment/platform
    const inferredOwnership: BuildFailureAnalysis['ownership'] =
      category === 'user_code' || category === 'dependency' || category === 'config' || category === 'runtime'
        ? 'user'
        : category === 'infra'
        ? 'platform'
        : category === 'network'
        ? 'environment'
        : 'unknown';
    const ownership = validOwnerships.includes(parsed.ownership as string)
      ? (parsed.ownership as BuildFailureAnalysis['ownership'])
      : inferredOwnership;
    const userActionable = typeof parsed.userActionable === 'boolean' ? parsed.userActionable : ownership === 'user';
    const platformActionable = typeof parsed.platformActionable === 'boolean' ? parsed.platformActionable : ownership === 'platform';
    return {
      category,
      ownership,
      summary: parsed.summary || `${step} 失敗：${errorMessage.slice(0, 80)}`,
      userFacingMessage: parsed.userFacingMessage || parsed.suggestedFix || '部署失敗，請查看下方詳情或聯繫管理員。',
      adminFacingMessage: parsed.adminFacingMessage || parsed.rootCause || errorMessage,
      userActionable,
      platformActionable,
      rootCause: parsed.rootCause || errorMessage,
      suggestedFix: parsed.suggestedFix || parsed.userFacingMessage || '請查看 Cloud Build / Cloud Run log 了解詳情',
