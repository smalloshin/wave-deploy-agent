...
export async function runPipeline(
  ...
  try {
    ...
    // `os.environ["JWT_SECRET"]` (KeyError on missing) or Pydantic
    // BaseSettings declarations with no default — neither of which
    // env-detector marked as REQUIRED. We catch them at scan time so
    // we don't waste Cloud Build minutes + GCS storage + Cloud Run quota.
    //
    // Python-only for now. Node/JS scanning can be added later (the
    // existing env-detector already covers most Node patterns reasonably).
    if (detection.language === 'python') {
      currentStep = 'Step 2.7: Required env vars';
      console.log(`[Pipeline] ${currentStep}...`);
      try {
        ...
        if (verdict.kind === 'block') {
          await transitionProject(projectId, 'failed', 'pipeline-worker', {
            ...
            error: `Required env vars missing: ${verdict.missingRequired.map((m) => m.name).join(', ')}`,
          });
          console.log(
            `[Pipeline]   blocked: ${verdict.missingRequired.length} required env vars missing — ${verdict.missingRequired.map((m) => m.name).join(', ')}`,
          );
          stopKeepAlive();
          return;
        }
        ...
        if (verdict.kind === 'auto-generated') {
          // Persist generated values into project.config.envVars (and remember
          // which names we generated for audit / display).
          const existing = (project?.config?.envVars as
            | string
            | Record<string, string>
            | undefined) ?? undefined;
          const merged = mergeEnvVarsR49(existing, verdict.generated);
          const { query: dbQuery2 } = await import('../db/index');
          await dbQuery2(
            `UPDATE projects SET config = config || $1::jsonb, updated_at = NOW() WHERE id = $2`,
            [
              JSON.stringify({
                envVars: merged,
                autoGeneratedSecrets: verdict.names,
              }),
              projectId,
            ],
          );
          console.log(
            `[Pipeline]   auto-generated ${verdict.names.length} secret(s): ${verdict.names.join(', ')}`,
          );
        }
      } catch (err) {
        // Non-fatal: env-gate is defense-in-depth. If the scanner itself
        // throws (file system error, etc.), continue — the deploy-worker
        // will hit the same gate again post-LLM.
        console.warn(
          `[Pipeline]   R49: env-gate failed (non-fatal, deploy-worker will re-check): ${(err as Error).message}`,
        );
      }
    }
